...
void main()
{
    ...
    // 바람?
    vec2 uv = v_vTexcoord + vec2(0.0, v_pixelsize.y * scrollSpeed);
    ...
    #endif
    float neighbor = neighbor2D(u_source, uv + flowmap);
    
    // 냉각맵 (텍스쳐) 값 가져오기
    float offy = (v_vTexcoord.y + v_pixelsize.y * scrollSpeed * u_time); // fract로 offy 가 0..1 범위에 있게 만들어요
    
    #ifdef USE_EXTERNAL_NOISE
        float coolmapRaw = texture2D(u_coolingmap, vec2(v_vTexcoord.x, fract(offy))).r;
        // 냉각맵 감마 조정
        float coolmap = smoothstep(0.0, 1.0, coolmapRaw * coolmapRaw);
        coolmap *= coolmap;
        coolmap *= 0.75;
    #else
        float coolmapRaw = fbm((vec2(v_vTexcoord.x, offy) + flowmap) * 30.0);
        // 냉각맵 감마 조정
        float coolmap = smoothstep(0.0, 1.0, coolmapRaw * coolmapRaw);
        coolmap *= 0.25;
    #endif
    ...
    // 적용
    float cooloffwankers = max(neighbor - coolmap, 0.0);
    ...
    */
    ...
    vec4 fin = vec4(vec3(cooloffwankers), 1.0);
    gl_FragColor = fin;
}
