...
varying vec2 v_vTexcoord;
varying vec4 v_vColour;
varying vec2 v_pixelsize; // 텍스쳐 공간 내에서 한 픽셀의 크기
...
uniform float u_scrollspeed; // 불이 올라가는 속도
uniform float u_windstrength; // 바람의 세기
uniform float u_windspeed; // 바람의 속도
...
// 주어진 픽셀과 이웃하는 픽셀의 평균값을 가져오는 함수 (R & B 채널)
...
void main()
{
    ...
    */
    ...
    float fireY = (v_vTexcoord.y + v_pixelsize.y * u_scrollspeed * u_time);
        ...
        float coolmapRaw = texture2D(u_coolingmap, vec2(v_vTexcoord.x, fract(fireY)) + flowmap).r; // fract로 fireY 가 0..1 범위에 있게 만들어요
        // 냉각맵 조정
